    ...
    public Bundle getBrowsableRootExtras() {
        ...
        try {
            ...
            for (Iterator<String> it = json.keys(); it.hasNext();) {
                String key = it.next();
                // Check boolean BEFORE int: Android's JSONObject.getInt()
                // coerces booleans (true→1, false→0) without throwing,
                // which breaks Bundle.getBoolean() lookups (e.g. Android
                // Auto's SEARCH_SUPPORTED flag).
